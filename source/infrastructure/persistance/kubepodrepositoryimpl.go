...
type kubePodRepository struct {
	...
	hostInfo           configuration.HostInfo
}

func InitKubePodRepository(ce executer.ISshCommandExecuter, hi configuration.HostInfo) repository.IKubePodRepository {
...
func (r *kubePodRepository) GetPodInfo(podName string, namespace string) string {
	var stringResult = r.sshCommandExecuter.RunSshCommand(r.hostInfo.KubernetesHost, r.hostInfo.HostUsername, r.hostInfo.HostPassword, "kubectl", "get pod "+podName+"  -n "+namespace+" -o json")
	return string(stringResult)
}

func (r *kubePodRepository) RestartPod(podName string, namespace string) bool {
	var stringResult = r.sshCommandExecuter.RunSshCommand(r.hostInfo.KubernetesHost, r.hostInfo.HostUsername, r.hostInfo.HostPassword, "kubectl", "delete pod "+podName+"  -n "+namespace)
...
func (r *kubePodRepository) GetNodes() *[]entity.Node {
	...
	var stringResult = r.sshCommandExecuter.RunSshCommand(r.hostInfo.KubernetesHost, r.hostInfo.HostUsername, r.hostInfo.HostPassword, "kubectl", "get nodes -o jsonpath='{.items[*].status.addresses[?(@.type==\"InternalIP\")].address}'")
...
func (r *kubePodRepository) GetNamespaces() *[]entity.Namespace {
	...
	var stringResult = r.sshCommandExecuter.RunSshCommand(r.hostInfo.KubernetesHost, r.hostInfo.HostUsername, r.hostInfo.HostPassword, "kubectl", "get namespaces")
...
func (r *kubePodRepository) GetDeployments(namespace string) *[]entity.Deployment {
	...
	var stringResult = r.sshCommandExecuter.RunSshCommand(r.hostInfo.KubernetesHost, r.hostInfo.HostUsername, r.hostInfo.HostPassword, "kubectl", "get deployments -n "+namespace)
	...
	for _, deploymentInfo := range deploymentInfos[1:] {
		...
		if deploymentFields != nil && len(deploymentFields) >= 4 {
			channelLength++
			go func() {
				containerInfoResults <- GetDeploymentInfo(r.sshCommandExecuter, r.hostInfo, deploymentFields, namespace)
			}()
		}
	}
...
func GetDeploymentInfo(sshCommandExecuter executer.ISshCommandExecuter, hostInfo configuration.HostInfo, deploymentFields []string, namespace string) *deploymentInfoResult {
	cmdResult := sshCommandExecuter.RunSshCommand(hostInfo.KubernetesHost, hostInfo.HostUsername, hostInfo.HostPassword, "kubectl",
...
func (r *kubePodRepository) UpdateImageForDeployment(deploymentName string, containerName string, newImage string, namespace string) bool {
	var stringResult = r.sshCommandExecuter.RunSshCommand(r.hostInfo.KubernetesHost, r.hostInfo.HostUsername, r.hostInfo.HostPassword,
...
func (r *kubePodRepository) GetByNamespace(namespace string) *[]entity.KubePod {
	...
	var stringResult = r.sshCommandExecuter.RunSshCommand(r.hostInfo.KubernetesHost, r.hostInfo.HostUsername, r.hostInfo.HostPassword, "kubectl", "get pods -n "+namespace)
...
func (r *kubePodRepository) GetAll() *[]entity.KubePod {
	...
	var nameSpacesResult = r.sshCommandExecuter.RunSshCommand(r.hostInfo.KubernetesHost, r.hostInfo.HostUsername, r.hostInfo.HostPassword, "kubectl", "get namespaces")
	...
	for _, nameSpace := range namespaceInfos[1:] {
		...
		if nameSpaceFields != nil && len(nameSpaceFields) >= 2 {
			var stringResult = r.sshCommandExecuter.RunSshCommand(r.hostInfo.KubernetesHost, r.hostInfo.HostUsername, r.hostInfo.HostPassword, "kubectl", "get pods -n "+nameSpaceFields[0])
			podInfos := strings.Split(string(stringResult), "\n")
			for _, podInfo := range podInfos[1:] {
				podFields := strings.Fields(podInfo)
				if podFields != nil && len(podFields) > 2 {
